...
const HRBatchPage: React.FC = () => {
  ...
  const successfulResults = useMemo(
    () => Object.values(results).filter((r) => r.success),
    [results]
  );
  ...
  const handleStartAnalysis = async () => {
    ...
    if (newFiles.length === 0) {
      toast({
        ...
        description: "All uploaded resumes are already analyzed.",
      });
      return;
    }
    ...
    try {
      ...
      if (cleanedQs.length)
        form.append("hrQuestions", JSON.stringify(cleanedQs));
      ...
      toast({
        ...
        description: `Successfully analyzed ${list.length} resume(s).`,
      });
    } catch (e: unknown) {
  ...
  const addQuestion = () => {
    ...
    if (questions.length >= 5) {
      toast({
        title: "Limit reached",
        description: "You can add up to 5 questions.",
        variant: "destructive",
      });
      return;
    }
    if (
      questions.some((existing) => existing.toLowerCase() === q.toLowerCase())
    ) {
      toast({
        title: "Duplicate question",
        description: "This question is already added.",
        variant: "destructive",
      });
      return;
    }
  ...
  const downloadAllReports = () => {
    ...
    if (successfulReports.length === 0) {
      toast({
        ...
        description: "No successful analyses with reports found.",
      });
      return;
    }
    ...
    a.href = URL.createObjectURL(file);
    a.download = `batch-analysis-${jobTitle
      .replace(/[^a-z0-9]/gi, "-")
    ...
    toast({
      ...
      description: `Combined report with ${successfulReports.length} analyses.`,
    });
  };
  ...
  const downloadReportAsPDF = async (name: string, report?: string) => {
    ...
    try {
      ...
      const htmlContent = report
        .replace(
          /^# (.*$)/gim,
          '<h1 style="font-size: 24px; font-weight: bold; margin: 20px 0 10px 0; color: #1f2937;">$1</h1>'
        )
        .replace(
          /^## (.*$)/gim,
          '<h2 style="font-size: 20px; font-weight: bold; margin: 16px 0 8px 0; color: #374151;">$1</h2>'
        )
        .replace(
          /^### (.*$)/gim,
          '<h3 style="font-size: 18px; font-weight: bold; margin: 12px 0 6px 0; color: #4b5563;">$1</h3>'
        )
        .replace(
          /^\*\*(.*)\*\*/gim,
      ...
      tempDiv.innerHTML = `
            <div style="max-width: 100%;">
                <div style="text-align: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #e5e7eb;">
                    <h1 style="font-size: 28px; font-weight: bold; color: #1f2937; margin: 0;">${cleanName(
                      name
                    )} - Analysis Report</h1>
    ...
    try {
      ...
      const coverPage = `
            <div style="text-align: center; margin-bottom: 40px; padding-bottom: 30px; border-bottom: 3px solid #3b82f6;">
                ...
                ${
                  jobLink
                    ? `<p style="color: #6b7280; margin: 10px 0;"><a href="${jobLink}" style="color: #3b82f6;">${jobLink}</a></p>`
                    : ""
                }
                    ...
                    <div style="page-break-before: ${
                      index > 0 ? "always" : "auto"
                        ...
                        <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #3b82f6;">
                            <h2 style="font-size: 22px; font-weight: bold; color: #1f2937; margin: 0 0 8px 0;">${
                              index + 1
                            }. ${cleanName(r.name)}</h2>
                            ${
                              r.score != null
                                ? `<p style=\"font-size: 16px; color: #059669; font-weight: 600; margin: 0;\">Compatibility Score: ${Math.round(
                                    r.score
                                  )}%</p>`
                                : ""
                            }
      ...
      pdf.save(
        `batch-analysis-${jobTitle.replace(/[^a-z0-9]/gi, "-").toLowerCase()}-${
      ...
      toast({
        title: "PDF downloaded",
        description: `Combined report for ${successfulReports.length} candidates.`,
      });
    } catch (error) {
      console.error("PDF generation failed:", error);
      toast({
  ...
  const canAnalyze =
    jobTitle.trim() &&
    (jobLink.trim() || jobDescription.trim()) &&
    files.length > 0 &&
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
        ...
        <div className="relative px-4 py-8 sm:py-12">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto">
              Upload multiple resumes and get instant AI analysis with
              compatibility scores for your job opening
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-12 mt-5">
        ...
        <UploadedFilesList
          ...
          onOpenReport={(name, report) =>
            setPreview({ open: true, name, report: report || null })
          }
                ...
                <span className="inline-flex items-center gap-3">
                  ...
                  <span>
                    {!hasNewFiles
                      ...
                      : `Analyze ${
                          files.filter((f) => !results[fileId(f.file)]).length
                        } Resume${
                          files.filter((f) => !results[fileId(f.file)])
                            .length !== 1
                            ? "s"
                            : ""
                        }`}
            ...
            {!canAnalyze && files.length > 0 && (
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-3">
                ...
                {jobTitle.trim() &&
                  !jobLink.trim() &&
                  !jobDescription.trim() &&
                  "Job link or description is required"}
